...
async function main() {
  ...
  const CarbonContract = await ethers.getContractFactory('Carbon');
  const carbonContract = await CarbonContract.deploy('0x8626f6940e2eb28930efb4cef49b2d1f2c9c1199');
  await carbonContract.deployed();
  saveFrontendFiles(carbonContract, "CarbonContract");
  console.log('Carbon deployed to:', carbonContract.address);
}
